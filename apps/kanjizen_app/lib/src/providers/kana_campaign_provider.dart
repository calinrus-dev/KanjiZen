...
final kanaCampaignProvider =
    StateNotifierProvider<
      KanaCampaignNotifier,
      AsyncValue<List<KanaLevelModel>>
    >((ref) {
      return KanaCampaignNotifier(ref.watch(campaignRepositoryProvider));
    });

class KanaCampaignNotifier
    extends StateNotifier<AsyncValue<List<KanaLevelModel>>> {
    ...
    for (int lvl = 1; lvl <= 100; lvl++) {
      ...
      if (lvl <= 50) {
        ...
        if (isBoss) {
          ...
          targets.addAll(
            hiras.sublist(startIdx, endIdx).map((k) => k.character),
          );
        } else {
          final startIdx = ((lvl - 1 - (lvl ~/ 5)) * 4).clamp(
            0,
            hiras.length - 1,
          );
        ...
        if (isBoss) {
          ...
          targets.addAll(
            katas.sublist(startIdx, endIdx).map((k) => k.character),
          );
        } else {
          final startIdx = ((relLvl - 1 - (relLvl ~/ 5)) * 4).clamp(
            0,
            katas.length - 1,
          );
